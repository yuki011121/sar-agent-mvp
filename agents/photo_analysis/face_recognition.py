    ...
    def detect_faces(self, image: np.ndarray, person_bbox: Optional[List[float]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                # DeepFace.extract_faces returns a list of numpy arrays (face images)
                ...
                for idx, face_image in enumerate(face_images):
                    # face_image is a numpy array (RGB format)
                    # We need to estimate the bounding box since extract_faces doesn't return bbox
                    face_info = {
                        "face_id": idx,
                        "region": face_image,  # Face image as numpy array
                        "bbox": None,  # Will be estimated below
                        "confidence": 1.0  # DeepFace doesn't provide confidence for detection
                    }
                    faces.append(face_info)
                
                # If we have person_bbox, adjust face coordinates
                if person_bbox and faces:
                    x1, y1, _, _ = map(int, person_bbox)
                    # Estimate face bbox (typically in upper portion of person)
                    for face in faces:
                        # Rough estimate: face is usually in upper 1/3 of person region
                        person_height = int(person_bbox[3]) - int(person_bbox[1])
                        face_height = int(person_height * 0.3)
                        face_width = int(face_height * 0.75)  # Typical face aspect ratio
                        face_x = x1 + (int(person_bbox[2]) - int(person_bbox[0]) - face_width) // 2
                        face_y = y1 + int(person_height * 0.1)
                        
                        face["bbox"] = [face_x, face_y, face_x + face_width, face_y + face_height]
